...
function checkReferences(filePath) {
  ...
  //to hold references to external adoc files
  var externalRefs = [];
  ...
  reader.on('end', function () {
    ...
    if (internalRefs[0] == null) {
      ...
      if (cannotFindInternal[0] == null) {
        atom.notifications.addSuccess(
          'All internal references are `OK`.'
        );
      } else {
    ...
    if (externalRefs[0] == null) {
        atom.notifications.addSuccess('No external reference found.');
    } else {
      ...
      forEachPromise(externalRefs, logItem).then(() => {
        if (allGood) {
          atom.notifications.addSuccess(
            'All external references are `OK`.'
          );
        }
      });
    }
