...
function forEachPromise(refs, fnCheck) {
  return refs.reduce(function (promise, item) {
...
function checkReferences(filePath, basePath) {
  ...
  reader.on('line', function (line, index, start, end) {
    ...
    if (line.match(/link:[^\[]+\[[\]]*\]/g)) {
      //console.log('LINE-----',line)
      var extractLink = line.match(/link:[^\[]+\[/g); //there may be more than one matching items
      for (var i = 0; i < extractLink.length; i++) {
        var newReference = extractLink[i];
    ...
    //find URLs that don't use the "link:"" macro
    //find external references that use the link:URI[some description] syntax
    if (line.match(/(^|[ \t]+)(http|https):\/\/[^ ]+[^\. ]/g)) {
      //console.log('LINE-----',line)
      var extractLink = line.match(/(http|https):\/\/[^ ]+[^\. ]/g); //there may be more than one matching items
      for (var i = 0; i < extractLink.length; i++) {
        var newReference = extractLink[i];
  ...
  reader.on('end', function () {
    ...
    if (externalRefs[0] == null) {
      ...
      // no bad refs found in current file
      var allGoodExtRef = true;
      ...
      forEachPromise(externalRefs, checkExternalRef).then(() => {
        if (allGoodExtRef) {
          atom.notifications.addSuccess(
    ...
    //check if each URL is reachable (if any)
    if (externalURLs[0] == null) {
        atom.notifications.addSuccess(
          '`' + logMsgFilePath + '`: No URLs found.');
    } else {
      // no bad refs found in current file
      var allGoodURL = true;
      ...
      function checkExternalURL(item) {
        ...
        return new Promise((resolve, reject) => {
            process.nextTick(() => {
              urlExists(item, function (err, exists) {
                //console.log(item, ' : ', exists);
                if (exists === false) {
                  allGoodURL = false;
                  atom.notifications.addError(
                    '`' + logMsgFilePath +
                    '`: Cannot reach URL: `' + item + '`',
                    {
                      dismissable: true,
                    });
                }
      ...
      forEachPromise(externalURLs, checkExternalURL).then(() => {
        if (allGoodURL) {
          atom.notifications.addSuccess(
            '`' + logMsgFilePath + '`: All URLs are reachable.'
          );
        }
      });
    }
